...
startup cost (~2-3s on Windows) on the critical path, so no TIMEOUT DQs.
...
def _mint_raw(agent_id: str, capability: str) -> str:
    ...
    with _signer_lock:
        ...
        resp_line = _signer_proc.stdout.readline().decode().strip()

        if not resp_line:
            # Process died mid-flight — restart and retry once
            _signer_proc = _start_signer()
            _signer_proc.stdin.write(req.encode())
            _signer_proc.stdin.flush()
            resp_line = _signer_proc.stdout.readline().decode().strip()
